...
const CSV_COLUMNS = [
  ...
  'lost_messages',
  'duplicate_messages',
  'global_ordering_violations',
  'native_scope_ordering_violations',
  'maximum_observed_backlog',
  'final_observed_backlog',
...
export function serializeSuiteCsv(suite: Suite): string {
  const rows = suite.runs.map((trial) => {
    ...
    return [
      ...
      suite.description ?? '',
      suite.status,
      trial.comparisonTrack,
      trial.position,
      trial.combinationIndex,
      trial.repetition,
      suite.configuration.sweep?.parameter ?? '',
      trial.sweepValue ?? '',
      trial.combination.broker,
      trial.combination.scenario,
      run?.configuration.consumerDelayMs ??
        suite.configuration.workload.consumerDelayMs,
      run?.id ?? '',
      run?.name ?? '',
      run?.description ?? '',
      run?.status ?? 'pending',
      ...
      metrics?.ordering.globalViolations ?? '',
      metrics?.ordering.nativeScopeViolations ?? '',
      metrics?.backlog.maximumObservedMessages ?? '',
      metrics?.backlog.finalObservedMessages ?? '',
      0,
