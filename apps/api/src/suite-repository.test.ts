...
describe('SuiteRepository', () => {
  ...
  it('persists the complete order and updates suite-run membership', () => {
    const suite = suites.create('Repository suite', configuration, order);
  ...
  it('records errors and stops interrupted suites during recovery', () => {
    const pending = suites.create('Pending', configuration, order);
    const running = suites.create('Running', configuration, order);
    suites.updateStatus(running.id, 'running');
    const completed = suites.create('Completed', configuration, order);
  ...
  it('rejects an incomplete execution order transactionally', () => {
    expect(() =>
      suites.create('Invalid', configuration, order.slice(0, 1)),
